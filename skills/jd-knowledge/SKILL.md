---
name: jd-knowledge
description: 供 AgentHub 智能体通过 JD Knowledge MCP 在指定用户和 Workspace 内新增文件或字符串、列出资源、检索知识、删除文件并跟踪异步任务。
---

# JD Knowledge（AgentHub）
...
`user_id`、`workspace_id`、`workspace_name` 必须由前端或调用方明确提供。JD Knowledge 不根据其中任何两个字段生成第三个字段，也不要让智能体自行猜测 Workspace ID。新建 Workspace 时，缺少任一字段就停止调用并向上游索取。

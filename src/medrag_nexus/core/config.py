...
class Settings(BaseSettings):
    ...
    # ========== 应用基础配置 ==========
    app_mode: Literal["dev", "prd"] = "dev"
    dev_database_name: str = "MedRAG-Nexus-dev"
    prd_database_name: str = "MedRAG-Nexus-prd"
    ...
    webui_elasticsearch_workspace_index: str = "medrag-nexus-dev-webui-workspaces"
    webui_elasticsearch_document_index: str = "medrag-nexus-dev-webui-resources"
    webui_elasticsearch_chunk_index: str = "medrag-nexus-dev-webui-chunks"
    webui_milvus_host: str = ""
    webui_milvus_port: int = 0
    webui_milvus_token: str = ""
    webui_milvus_collection: str = "medrag_nexus_dev_webui_chunks"
    webui_redis_url: str = ""
    webui_redis_queue_name: str = "medrag-nexus:dev:webui:tasks"
    ...
    elasticsearch_workspace_index: str = "medrag-nexus-dev-workspaces"  # Workspace 冗余索引
    elasticsearch_document_index: str = "medrag-nexus-dev-resources"  # 资源冗余索引
    elasticsearch_chunk_index: str = "medrag-nexus-dev-chunks"  # 文本块索引
    legacy_elasticsearch_indices: str = ""

    # ========== Redis 任务队列配置 ==========
    redis_url: str = "redis://127.0.0.1:20000/0"  # 固定 IPv4，避免 localhost 优先解析到未监听的 ::1
    worker_concurrency: int = 4
    redis_queue_name: str = "medrag-nexus:dev:tasks"
    ...
    sqlite_path: Path = Path("./data/MedRAG-Nexus-dev/MedRAG-Nexus-dev.sqlite3")
    migration_marker: str = "medrag_nexus_dev_storage_initialized"
    legacy_milvus_collection: str = "medrag_nexus_dev_legacy_chunks"
    ...
    def apply_app_mode_storage_namespace(self) -> Settings:
        """为未显式覆盖的存储配置应用 dev/prd 隔离命名空间。"""

        database_name = self.database_name
        search_prefix = database_name.casefold()
        vector_prefix = search_prefix.replace("-", "_")
        data_root = (Path("./data") / database_name).resolve()
        webui_data_root = data_root / "webui"
        defaults: dict[str, object] = {
            "data_root": data_root,
            "sqlite_path": data_root / f"{database_name}.sqlite3",
            "elasticsearch_workspace_index": f"{search_prefix}-workspaces",
            "elasticsearch_document_index": f"{search_prefix}-resources",
            "elasticsearch_chunk_index": f"{search_prefix}-chunks",
            "milvus_collection": f"{vector_prefix}_chunks",
            "redis_queue_name": f"medrag-nexus:{self.app_mode}:tasks",
            "migration_marker": f"{vector_prefix}_storage_initialized",
            "legacy_elasticsearch_indices": "",
            "legacy_milvus_collection": f"{vector_prefix}_legacy_chunks",
            "webui_data_root": webui_data_root,
            "webui_sqlite_path": webui_data_root / f"{database_name}-webui.sqlite3",
            "webui_elasticsearch_workspace_index": f"{search_prefix}-webui-workspaces",
            "webui_elasticsearch_document_index": f"{search_prefix}-webui-resources",
            "webui_elasticsearch_chunk_index": f"{search_prefix}-webui-chunks",
            "webui_milvus_collection": f"{vector_prefix}_webui_chunks",
            "webui_redis_queue_name": f"medrag-nexus:{self.app_mode}:webui:tasks",
            "webui_audit_log_dir": data_root / "audit" / "webui",
        }
    ...
    def webui_runtime_settings(self) -> Settings:
        ...
        return self.model_copy(
            update={
                ...
                "migration_marker": f"{self.storage_key}_webui_storage_initialized",
                "legacy_elasticsearch_indices": "",
                "legacy_milvus_collection": f"{self.storage_key}_webui_no_legacy_collection",
            }
        )
    ...
    def database_name(self) -> str:
        ...
        return self.dev_database_name if self.app_mode == "dev" else self.prd_database_name

    @property
    def storage_key(self) -> str:
        """适用于 Milvus、迁移标记等标识符的安全命名空间。"""

        return self.database_name.casefold().replace("-", "_")

...
from jd_knowledge.core.models import TaskRecord
        ...
        try:
            async with httpx.AsyncClient(
                ...
                response = await client.post(
                    ...
                    json=payload,
                    headers={
                        "X-JD-Knowledge-Event": str(payload["event"]),
                        "X-JD-Knowledge-Task-ID": str(payload["task_id"]),
                    },
                )
...
def schedule_task_callback(runtime: Runtime, task: TaskRecord) -> None:
    ...
    asyncio.create_task(
        ...
        name=f"jd-knowledge-callback-{task.task_id}-{task.status.value}",
    )

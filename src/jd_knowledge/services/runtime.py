...
from jd_knowledge.pipeline.models import EmbeddingClient, RerankClient
from jd_knowledge.services.task_log import TaskLogStore
from jd_knowledge.storage.elasticsearch import ElasticsearchStore
...
class Runtime:
    ...
    async def start(self, *, initialize_storage: bool = True, start_workers: bool = True) -> None:
        ...
        if start_workers:
            ...
            self.worker_tasks = [
                asyncio.create_task(self._worker_loop(index), name=f"jd-knowledge-worker-{index}")
                for index in range(self.settings.worker_concurrency)
            ]
            self.maintenance_task = asyncio.create_task(self._maintenance_loop(), name="jd-knowledge-maintenance")
    ...
    async def _worker_loop(self, index: int) -> None:
        ...
        heartbeat_task = asyncio.create_task(beat(), name=f"jd-knowledge-heartbeat-{index}")
        dequeue_failures = 0
        await self._log_worker(
            "INFO",
            "Worker 已启动",
    ...
    async def _maintenance_loop(self) -> None:
        from jd_knowledge.services.processing import recover_interrupted_task

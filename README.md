...
2. 启动 FastAPI、FastMCP、Public/WebUI Runtime、Redis Worker 与维护任务；
...
| 公开数据 | `DATA_ROOT`、`SQLITE_PATH`、`ELASTICSEARCH_*_INDEX`、`MILVUS_COLLECTION` | 公开 API/MCP 使用 |
| WebUI 数据 | `WEBUI_DATA_ROOT`、`WEBUI_SQLITE_PATH`、`WEBUI_ELASTICSEARCH_*_INDEX`、`WEBUI_MILVUS_COLLECTION` | 与公开数据隔离 |
...
WebUI 默认复用公共 Elasticsearch、Milvus、Redis 的连接地址和认证，只隔离 SQLite、文件目录、索引、Collection 与任务队列。需要单独基础设施时可配置 `WEBUI_ELASTICSEARCH_*`、`WEBUI_MILVUS_*` 和 `WEBUI_REDIS_URL`。
...
`WEBUI_LOCK_PASSWORD` 只保护 WebUI BFF，不影响公开 `/api/v1/*` 和 `/mcp`。默认不信任 `X-Forwarded-For`；只有受控反向代理会覆盖或追加代理头时，才设置：
...
公开 API 与 FastMCP 共用 Public Runtime、SQLite、Elasticsearch、Milvus 和文件目录。

| Surface | Path / Tool | 行为 |
| --- | --- | --- |
| HTTP | `POST /api/v1/add` | 新增 PDF、TXT、DOCX 或字符串，异步返回任务 |
| HTTP | Workspace / resource routes | 列表、改名、整库或整份资源删除 |
| HTTP | retrieval route | Elasticsearch BM25 + Milvus Vector + RRF + 可选 Rerank |
| HTTP | task route | 查询状态、阶段、进度、结果和错误 |
| HTTP | health routes | 存活检查与依赖就绪诊断 |
| MCP | `add` | 新增文件或字符串 |
...
新增和删除可通过任务接口轮询，也可提供 `callback_url` 接收状态、进度与结果。文件和字符串分别按 SHA-256 去重；文件 ID 创建后保持稳定。
...
一个 FastAPI 进程同时承载 HTTP、FastMCP、WebUI BFF、两套 Runtime、Worker 和维护任务：

| Runtime | 调用入口 | SQLite / 文件 | Elasticsearch / Milvus | Redis Queue |
| --- | --- | --- | --- | --- |
| Public Runtime | `/api/v1/*`、`/mcp` | `SQLITE_PATH`、`DATA_ROOT` | `ELASTICSEARCH_*_INDEX`、`MILVUS_COLLECTION` | `knowledge:tasks` |
| WebUI Runtime | Next.js → `/webui/*` BFF | `WEBUI_SQLITE_PATH`、`WEBUI_DATA_ROOT` | `WEBUI_ELASTICSEARCH_*_INDEX`、`WEBUI_MILVUS_COLLECTION` | `knowledge:webui:tasks` |

两套 Runtime 可以复用相同的 Redis、Elasticsearch、Milvus 和模型服务连接，但命名空间与本地权威数据完全分离，避免浏览器用户数据和外部 AgentHub 调用方混用。
